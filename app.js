...
let limit = parseInt(perPageSelect.value);
 ...
 loadTodosButton.addEventListener('click', async () => {
    ...
    perPageSelect.addEventListener("change", ({ target }) => {
        ...
        pageNumber = 0;
        loadTodosFreapi(pageNumber, limit, {
            empty: true,
        });
    });
 ...
 pagination.addEventListener("change", ({ target }) => {
...
await loadTodosFromApi(start, limit);
 ...
 async function loadTodosFreapi(start, limit,options) {
 ...
 function addEventListener(list, empty = false){
     const todoListdiv = document.querySelector(".todolist");
     const h2 = document.createElement("h2");
     h2.textContent = 'loaded - ${list,length}, todoId - ${list[0].id}';
     if (empty){
